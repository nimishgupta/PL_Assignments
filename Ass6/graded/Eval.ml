...
(* AG *)
type context = context' * exp_env
let is_empty_context (cxt, _) = is_empty_context' cxt
let empty_context : context = (empty_context', Env Env.empty)
let step (e : exp) ((cxt, env) : context) : exp * context = 
  let (exp, cxt', env) = step e cxt env in
  (exp, (cxt', env))