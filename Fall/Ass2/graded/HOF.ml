...
let rec eval_helper (binds : env) (e : exp) : value =
  match e with
    ...
    | Let (x, with_e, in_e) -> 
      (* AG: Terminology: binds is not the global environment. It is the
         current environment. There is usually just one unique global
         environement. *)
      (* 
      ...
      *)
       ...
       (* AG: You can create an identifier called binds' (no need to spell out
          prime *)
      ...
      (match (eval_helper binds e) with
        | Closure (idList, body, func_env) -> 
            ...
            let rec f (exps : exp list) : value list = 
              (* AG: As a matter of style, I recommend writing
                 match exps with
                 | [] -> ...
                 | _ -> ...
               *)
...
(* AG: ;; is never needed in normal code. despite what the internet says *)
module S = HOF_sugar;; 
...
let rec desugar (s_exp : S.exp) : exp =
  match s_exp with
    ...
    | S.Apply (e, params) -> 
    (* AG: Yeah, you could write a desugar_exp_list function *)
    ...
    (* List Semantics *)
    (* AG: I strongly approve of the (sort of) Church encoding. *)
    ...
    (* AG: This is basically OOP, where you have three "methods" called
       0, 1, and, 2. *)
